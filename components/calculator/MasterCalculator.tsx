    ...
    return (
      <div className="flex items-center justify-between rounded-xl border border-slate-200/90 bg-slate-50/60 p-4 transition-all hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-800/40 dark:hover:bg-slate-800/60">
        <label htmlFor={inputId} className="cursor-pointer pr-4 text-sm font-medium text-slate-700 dark:text-slate-200">
          ...
          {helpText && <span className="block mt-0.5 text-xs text-slate-500 dark:text-slate-300">{helpText}</span>}
        </label>
        <button
          id={inputId}
          type="button"
  ...
  if (input.type === 'select' && input.options) {
    return (
      <div>
        ...
        {helpText && (
          <p id={helpId} className="mt-1.5 text-xs text-slate-500 dark:text-slate-300">
            {helpText}
          </p>
        )}
      </div>
  ...
  if (input.type === 'slider') {
    return (
      <div>
        ...
        />
        {helpText && (
          <p id={helpId} className="mt-1 text-xs text-slate-500 dark:text-slate-300">
            {helpText}
          </p>
        )}
      </div>
    ...
    return (
      <div>
        ...
        <div className="mt-1.5 flex rounded-xl shadow-xs">
          ...
          />
          {trailingUnit && (
            <span className="inline-flex shrink-0 items-center rounded-r-xl border border-l-0 border-slate-300/90 bg-slate-50 px-3 text-xs font-semibold text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
              /{trailingUnit}
            </span>
          )}
        </div>
